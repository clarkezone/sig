    ...
    while (true) {
        ...
        for (available_snapshot_peers.items) |peer| {
            ...
            if (snapshot_type == .Full or snapshot_type == .All) { //TODO:ALL
                logger
                    .info()
                    .logf("reliabledownloading full_snapshot from: {s}", .{snapshot_url});
                ...
                logger
                    .info()
                    .logf("skipping reliabledownloading full_snapshot from: {s}", .{snapshot_url});
            }
...
pub fn downloadSnapshotsFromGossip(
    ...
    return reliablyDownloadSnapshotsFromGossip(allocator, logger_, maybe_trusted_validators, gossip_service, output_dir, min_mb_per_sec, SnapshopType.Incremental);
}
